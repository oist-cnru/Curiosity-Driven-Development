...
do
    temp_file=$(mktemp)
    singularity exec communication.sif python -c "from communication.bash.slurmcraft import all_like_this; result = all_like_this('$arg'); print(result, file=open('${temp_file}', 'w'))"
...
singularity exec communication.sif python communication/bash/slurmcraft.py --comp ${comp} --agents ${agents} --arg_list "${arg_list}"
